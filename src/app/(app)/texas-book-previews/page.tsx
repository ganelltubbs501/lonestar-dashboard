...
import { BookMarked, Loader2, Search, ExternalLink } from 'lucide-react';
...
function statusColor(val: string) {
  ...
  if (v === 'done' || v === 'complete' || v === 'completed' || v === 'yes') return 'bg-green-100 text-green-700';
  if (v === 'in progress' || v === 'in-progress') return 'bg-blue-100 text-blue-700';
  if (v === 'blocked' || v === 'on hold' || v === 'no') return 'bg-red-100 text-red-700';
...
export default function TexasBookPreviewsPage() {
  const [search, setSearch] = useState('');
  const [activeTab, setActiveTab] = useState(0);

  const { data, isLoading, error } = useFetch<TBPData>('/api/texas-book-previews', {
    pollInterval: 60000,
  });
